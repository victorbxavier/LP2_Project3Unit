...
public abstract class ManipuleData implements SimilarityAnalysis, PreProcessing {
    ...
    public double levDistance(String txt1, String txt2) {
        double greaterStr = Integer.max(txt1.length(), txt2.length());

        LevenshteinDistance levenshteinDistance = new LevenshteinDistance();
        double distance = levenshteinDistance.apply(txt1, txt2);

        return (greaterStr - distance) / greaterStr; //Indice Normalizado
    }
    ...
    public String cleanString(String originalText) {
        ...
        List<String> listStrings = new ArrayList<>(Arrays.asList(strArr));
    ...
    @Override
    public double testSimilarity(String text, int threehold) {

        List<News> fakenews = this.dao.findAllNews();
        double levdistance = 0;
        double jaroWinklerSimilarity = 0;
        double levdistanceBigger = 0;
        double jaroWinklerSimilarityBigger = 0;

        for (News news : fakenews) {
            levdistance = levDistance(text, news.getText_format());
            jaroWinklerSimilarity = jaroWinklerSimilarity(text, news.getText_format());
            if(levdistance > levdistanceBigger){
                levdistanceBigger=levdistance;
            }
            if (jaroWinklerSimilarity > jaroWinklerSimilarityBigger){
                jaroWinklerSimilarityBigger=jaroWinklerSimilarity;
            }
